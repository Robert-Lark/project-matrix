 ...
 * record, and this file asserts the vanilla implementation against the
 ...
 * survive a variant swap (ADR-0004 §5, same-origin storage).
 *
...
const PAGE = `${ORIGIN}/vanilla/editorial/`;
 ...
 * reload here would land synthetic RUM (variant=vanilla, surface=editorial)
 * in the production collector, unmarked (the bench-runner interception
 * precedent, collect.ts). chrome.browser.test.ts deliberately lets its
 * placeholder beacons land — that suite proves the collector; this one
 * proves the cart.
 */
async function openCartPage(context: BrowserContext): Promise<Page> {
