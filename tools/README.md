...
- `snapshot-fixture` — the committed CI seed (issue #4): 240 deterministic,
  clearly-synthesized, schema-valid placeholder releases + placeholder images
  + a dated manifest, regenerated byte-identically from a fixed PRNG seed
  (`pnpm --filter @pm/snapshot-fixture run generate`). It is what local dev
  and the origin suite seed into R2 by default, and it carries every rendering
  branch the real crate does, so CI's drift gate exercises them all without
  ever touching the crate or the Discogs API (ADR-0001 §9).
