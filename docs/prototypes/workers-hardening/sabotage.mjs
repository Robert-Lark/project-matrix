...
// ── Round 2 (2026-09-25, after verify-slice): one row per fix, plus the
// deferred E2, plus controls. Selected with `node sabotage.mjs R…`.
const R2 = [
  { id: "R0", control: true, defect: "CONTROL (round 2): the refusal suite, the edge suite and the three typechecks pass on the fixed tree", guard: `${V} && pnpm --filter @pm/edge exec vitest run && ${TC("edge")} && ${TC("front")} && ${TC("blog")}` },
  { id: "R1", defect: "edge: isFinite half of the value check dropped (typeof only) — the raw 1e999 legs must catch it", file: "workers/edge/src/index.js", edits: [["if (typeof event.value !== \"number\" || !Number.isFinite(event.value)) {", "if (typeof event.value !== \"number\") {"]], guard: "pnpm --filter @pm/edge exec vitest run test/beacon.test.js" },
  { id: "R2", defect: "publish.mjs: one `// refusal:` marker removed (a throw with no row binding)", file: "workers/front/lab/publish.mjs", edits: [["  // refusal: dirty-tree\n", ""]], guard: V },
  { id: "R3", defect: "publish.mjs: a marker relabelled to a name no row carries", file: "workers/front/lab/publish.mjs", edits: [["// refusal: cross-tree", "// refusal: cross-treee"]], guard: V },
  { id: "R4", defect: "front index.js: a sibling binding misspelt (BLOG → BLGO) — typed to Env now", file: "workers/front/src/index.js", edits: [["  blog: \"BLOG\",", "  blog: \"BLGO\","]], guard: TC("front") },
  { id: "R5", defect: "publish.mjs: a reading-table row named outside the switcher's union (\"INP\" for \"INP (scripted)\")", file: "workers/front/lab/publish.mjs", edits: [["([\"INP (scripted)\", \"INP\"])", "([\"INP\", \"INP\"])"]], guard: TC("front") },
  { id: "R6", defect: "publish.mjs: cc provenance drops the dirty-origin clause (skeptic M1)", file: "workers/front/lab/publish.mjs", edits: [["if (!oc || oc.dirty !== false || oc.sha !== chromeConstant.commit.sha) {", "if (!oc || oc.sha !== chromeConstant.commit.sha) {"]], guard: V },
  { id: "R7", defect: "publish.mjs: tolerance finiteness dropped (skeptic M5 — Infinity would never fire the constancy check)", file: "workers/front/lab/publish.mjs", edits: [["      Number.isFinite(declared.toleranceBytes) &&\n", ""]], guard: V },
  { id: "R8", defect: "publish.mjs: band overlap `>=` → `>` (skeptic M15 — touching bands would publish a ranking)", file: "workers/front/lab/publish.mjs", edits: [["if (lower.max >= upper.min && upper.max >= lower.min) {", "if (lower.max > upper.min && upper.max > lower.min) {"]], guard: V },
  { id: "R9", defect: "publish.mjs: provenance cutoff `>=` → `>` (skeptic M17 — the cutoff day itself skips attestation)", file: "workers/front/lab/publish.mjs", edits: [["if (day >= PROVENANCE_CUTOFF) {", "if (day > PROVENANCE_CUTOFF) {"]], guard: V },
  { id: "R10", defect: "publish.mjs: column axis exact → subset (skeptic M20 — a fifth variant admitted)", file: "workers/front/lab/publish.mjs", edits: [["if (measured.join(\",\") !== registered.join(\",\")) {", "if (!registered.every((v) => measured.includes(v))) {"]], guard: V },
  { id: "R11", defect: "publish.mjs: unsettled `!== true` → `=== false` (skeptic M23 — an ABSENT flag admits)", file: "workers/front/lab/publish.mjs", edits: [["if (run.interactionSettled !== true) {", "if (run.interactionSettled === false) {"]], guard: V },
  { id: "R12", defect: "publish.mjs: populated `!== true` → `=== false` (skeptic M26)", file: "workers/front/lab/publish.mjs", edits: [["if (chromeConstant.measuredChrome?.populated !== true) {", "if (chromeConstant.measuredChrome?.populated === false) {"]], guard: V },
  { id: "R13", defect: "publish.mjs: longTaskMs dropped from the finite-delta metric list (skeptic M27)", file: "workers/front/lab/publish.mjs", edits: [["for (const metric of [\"FCP\", \"LCP\", \"CLS\", \"longTaskMs\"]) {", "for (const metric of [\"FCP\", \"LCP\", \"CLS\"]) {"]], guard: V },
  { id: "R14", defect: "publish.mjs: encoding token no longer lower-cased — the upper-case CONTROL leg must catch the false refusal", file: "workers/front/lab/publish.mjs", edits: [["? attribution.contentEncoding.trim().toLowerCase()", "? attribution.contentEncoding"]], guard: V },
  { id: "R15", defect: "publish-refusals: the real-renderer chrome-constant control — chromeFragmentOf swapped for identity (a constant hashed from the whole page, not the fragment)", file: "workers/front/test/publish-refusals.test.js", edits: [["    const fragment = chromeFragmentOf(renderChrome({ ...rc, lab }));", "    const fragment = renderChrome({ ...rc, lab });"]], guard: V },
  { id: "R15b", defect: "publish-refusals: the real-renderer control hashes an UNPOPULATED render (lab: undefined) while the constant claims populated — the identity gate must refuse the control's own constant", file: "workers/front/test/publish-refusals.test.js", edits: [["    const fragment = chromeFragmentOf(renderChrome({ ...rc, lab }));", "    const fragment = chromeFragmentOf(renderChrome({ ...rc, lab: undefined }));"]], guard: V },
  { id: "R16", defect: "published-readings.test.ts: the switcher's READING_METRICS import replaced by a stale local copy (\"INP\") — the typecheck cannot see it; MISSED expected on the local guard, the suite's own leg is the catcher on a plane", file: "tools/origin-suite/suite/published-readings.test.ts", edits: [["import { READING_METRICS } from \"@pm/switcher\";", "const READING_METRICS = [\"initial JS\", \"TTFB\", \"FCP\", \"LCP\", \"CLS\", \"INP\"];"]], guard: "pnpm --filter @pm/origin-suite exec tsc --noEmit" },
